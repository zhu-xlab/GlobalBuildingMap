...
class PlanetScopeDataset_Prod(torch.utils.data.Dataset):
    ...
    def __init__(self, rootDir, status="train", patchSize=None, step=None, augmentation=True):
    ...
    def __getitem__(self, i):
        ...
        if (self.status == 'train'):
            data_p, label_p = load_image_tif_prod(self.nameFiles[i], self.dataDir, labelDir = self.labelDir)
        elif (self.status == 'val'):
            data_p, label_p = load_image_tif_prod(self.nameFiles[i], self.dataDir, labelDir = self.labelDir)
        else:
            data_p, label_p = load_patch(self.img, )
...
def metrics(target, predication, numClass):
    ...
    cm = confusion_matrix(gts, predictions, range(numClass))
