...
def planet_infer_normalize_ahcUpSolo_patch(filename, iqrC, pS=512):
    ...
    for idP, coords in enumerate(planet_infer_grouper(1, planet_infer_swCoords(img, step=pS, window_size=(pS, pS)))):
        x,y,w,h = coords[0]
...
def planet_infer_writeTiff(inTiff, outTiff, outData, dataType):
    ...
    if (dataType == 'uint32'):
        outDs = gdal.GetDriverByName('GTiff').Create(outTiff, cols, rows, 1, gdal.GDT_UInt32)
    else:
        outDs = gdal.GetDriverByName('GTiff').Create(outTiff, cols, rows, 1, gdal.GDT_Byte)
